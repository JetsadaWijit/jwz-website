---
name: design-system
description: Silver Glass — the visual language every published page follows: tokens, glass surfaces, components, layout chrome, and the runtime include system.
---
...
In this repository the site root is `docs/`, so every path below — `css/`, `js/`,
`partials/` — resolves under `docs/`.
